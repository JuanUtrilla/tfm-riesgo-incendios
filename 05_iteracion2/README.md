...
La pregunta de operación: «¿cuál de las 498,530 celdas arde hoy?». En esta
iteración se construyó un conjunto de entrenamiento que plantea esa pregunta
y ninguna otra.

## Los dos cambios

Respecto a la primera iteración cambiaron dos cosas del diseño: de dónde
salen los negativos y qué se etiqueta como positivo. La etiqueta pasó de la
ignición registrada en la EGIF (Estadística General de Incendios Forestales)
a la superficie quemada de EFFIS (*European Forest Fire Information System*).
...
si algo mejora, no se puede atribuir a información nueva ni al ajuste. La
partición es temporal: entrenamiento 2015-2022, validación 2023, test 2024.
Las temporadas 2025 y 2026 quedan como prueba externa (`06_comparacion/`).
...
| `52_muestreo/dos_12_muestrear_effis.py` | Tabla maestra con etiqueta EFFIS. Positivos: el primer día de cada celda quemada, con un tope de 30 por día y bloque de 100 km. Negativos: otras celdas del mismo día. Incluye también un banco de evaluación dentro del día (veranos 2023 y 2024) | El dataset de los modelos definitivos |
...
| `57_ablaciones/dos_08_verano.py` | Mide si entrenar solo con verano mejora el producto de verano | No: el DÓNDE empeora (−0.028) |
| `57_ablaciones/dos_17_capa_quemado.py` | Mide si apagar las celdas ya quemadas ayuda | No: entre −0.016 y −0.024 según la ventana |
...
El modelo único (`donde_dia_effis`) usa las 46 variables, negativos del mismo
día y etiqueta EFFIS. Obtuvo un AUC de 0.809 en test 2024 y de 0.752 sobre la
temporada 2026 con reanálisis; con diez negativos por positivo sube a 0.759 y
esa variante es el r10.

El modelo dónde (`donde_effis_c`) tiene una fila por celda y ninguna variable
meteorológica. Responde «¿esta celda es propensa?» y su mapa es el mismo todos
los días. El modelo cuándo usa solo las variables dinámicas, con negativos de
la misma celda en otros días, y responde «¿hoy es peligroso?». La pareja es el
producto de los dos: 0.705 en 2026.
...
El ratio de negativos merece una cautela. La ganancia en AUC medio (entre
+0.006 y +0.017 según el año) es del orden del ruido del sorteo de negativos
(0.004, medido repitiendo el sorteo con la misma receta). En la temporada 2026
el r10 fue el que mejor situó el fuego grande (el percentil ponderado por
hectáreas pasa de 79.5 con 1:3 a 81.9 con 1:10). En el replay de 2025 y 2026 el
1:3 y el 1:10 empatan en AUC. Se eligió el r10 con esa reserva.
...
cubo y de ahí salen tres resultados. El primero es una escala fija para el
mapa: en EXTREMO arde una celda de cada 800, 36 veces la tasa media. El
segundo es la medida de que la probabilidad por celda sirve para ordenar y
para dar la magnitud típica, pero no como valor absoluto por celda y día. El
tercero fue un aviso de día con umbral fijo, el semáforo. Funcionaba en
invierno y primavera (BSS, *Brier Skill Score*, de +0.30) y no en verano,
donde el IC (intervalo de confianza) cruza el cero. Se evaluó fuera de
calibración y se descartó (`07_produccion/escala_y_cifra/README.md`). El
detalle está en el README de `56_calibracion/`. La cadena diaria de
`07_produccion/` publica el mapa con la escala absoluta, recalibrada sobre los
mapas servidos, y la cifra del día.
