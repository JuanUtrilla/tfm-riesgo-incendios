...
dest = f"{AQUI}/salida_replay/{a.anio}/{a.condicion}"
t0 = time.time(); hechos = fallos = 0
for d in pd.date_range(ini, fin, freq="D"):
    f = str(d.date())
    if os.path.exists(f"{dest}/{f}.npz"):
        continue
    r = subprocess.run([P, f"{AQUI}/replay_dia_todo.py", "--fecha", f, "--condicion", a.condicion, "--sin-png"],
                       env=env, capture_output=True, text=True)
    if r.returncode != 0:
        fallos += 1
        open(f"{AQUI}/salida_replay/fallos_{a.anio}_{a.condicion}.log", "a").write(f"=== {f}\n{r.stdout[-1500:]}\n{r.stderr[-3000:]}\n")
