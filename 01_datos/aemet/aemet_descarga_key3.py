...
# ── Configuración ───────────────────────────────────────────────────────────────
API_KEY    = os.environ["TOKEN_AEMET_3"]
BASE_URL   = "https://opendata.aemet.es/opendata/api"
DB_PATH    = "aemet_historico.db"
IDEMA_DESDE = "6172X"  # key3 arranca en el último tercio + Baleares + Canarias
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s %(levelname)s [KEY3] %(message)s",
    handlers=[
        logging.FileHandler("aemet_descarga_key3.log"),
...
def ordenar_estaciones(estaciones):
    ...
    # Key3: primero su tramo asignado (desde IDEMA_DESDE en adelante, incluye B y C);
    # al agotarlo, vuelve al principio para ayudar con lo que quede pendiente
    # (ya_descargado()/todos_sin_datos() saltan rápido lo que otra key ya hizo).
    idx = next((i for i, e in enumerate(ordenadas) if e["indicativo"] >= IDEMA_DESDE), 0)
    return ordenadas[idx:] + ordenadas[:idx]
