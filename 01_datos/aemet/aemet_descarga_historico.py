...
# ── Configuración ───────────────────────────────────────────────────────────────
API_KEY    = os.environ["TOKEN_AEMET"]
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("aemet_descarga.log"),
...
def ordenar_estaciones(estaciones):
    """
    Peninsulares (idema numérico) primero, que son las que más datos tienen.
    Luego Baleares (B), luego Canarias (C).
    Dentro de cada grupo, orden por idema para consistencia.
    """
    def prioridad(e):
        p = e["indicativo"][0]
    ...
    return sorted(estaciones, key=prioridad)
