...
hoy o mañana necesita la previsión del módulo 2b, que aún no existe. En
retrospectivo no hace falta 2b y se puede comparar contra el cubo, que es la
referencia.
...
Dos limitaciones, las dos medidas antes de aceptarlas
-----------------------------------------------------
...
import config
import features
from features import FULL, METEO_MALLA
...
MODELO = f"{config.MODELOS}/xgb_v1.ubj"
...
def meteo_malla(d, mes, permitir_descarga, mapear=False):
    ...
    def hay(y, m):
        n = f"era5land_{y}{m:02d}.nc"
        return any(os.path.exists(f"{b}/{n}")
                   for b in (config.CRUDO_PROPIO, config.CRUDO))

    faltan = [(p.year, p.month) for p in meses if not hay(p.year, p.month)]
    ...
    rutas, extraidos = [], []
    for p in meses:
        if hay(p.year, p.month) or permitir_descarga:
...
def a_celdas(F, idx_nodo):
    """Cada celda toma el valor de su nodo. Sin IDW, sin pesos ni vecinos."""
...
def compartidas(ds, d, t, anio, mes, dia_anio, ny, nx, xs, ys):
    ...
    firms = pd.read_parquet(config.FIRMS,
    ...
    wglc = xr.open_dataset(config.WGLC)
...
def main(a):
    ...
    features.verifica()          # que las columnas no hayan divergido
    ...
    salida = config.salida(f"mapa_riesgo_{a.fecha}.png")
    fig.savefig(salida, dpi=140, bbox_inches="tight")

    with open(config.salida(f"riesgo_{a.fecha}.json"), "w") as f:
        json.dump(res, f, indent=1, ensure_ascii=False)
    ds.close()
    print(f"\nGuardado: {salida}")
    print(f"          {config.salida(f'riesgo_{a.fecha}.json')}")
